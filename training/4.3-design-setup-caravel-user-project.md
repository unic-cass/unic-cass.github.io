...
1. Download the source code with the correct release/tag. In this tutorial, we will use the latest release tag of Caravel which is mpw-9j as follows:

```bash
git clone -b mpw-9j https://github.com/efabless/caravel_user_project \
uniccass_example
cd uniccass_example
```
...
1. Next, we need to set up two environment variables `PDK`. This is needed during the setup because `caravel_user_project` use a specific version of Skywater 130nm PDK and a specific version of Openlane.
...
git clone -b mpw-9j https://github.com/efabless/caravel-lite /home/cass/uniccass_example/caravel --depth=1
