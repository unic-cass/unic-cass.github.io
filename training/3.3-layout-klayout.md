...
The current version of OpenPDK for Sky130A has some bugs in the configuration file when it is used with `Klayout` even with the tweak above. Thefore, you have to run the following two commands to tweak the tech files for `Klayout`:
...
The first command will delete this line `<?xml version="1.0" encoding="utf-8"?>` in `libs.tech/klayout/pymacros/sky130.lym`

The second command will replace the string `sky130` into the string `sky130A` in `libs.tech/klayout/tech/sky130A.lyt`
