...
> ## Lecture video
> 
> <iframe width="560" height="315" src="https://www.youtube.com/embed/PN1tbY9R4Hc" title="2.1 Digital design tools on Linux or WSL using Docker" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
*Command output*:
```
cass@UNIC:~$ sudo apt-get install ca-certificates curl gnupg -q -y
...
cass@UNIC:~$ sudo install -m 0755 -d /etc/apt/keyrings
cass@UNIC:~$ curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
cass@UNIC:~$ sudo chmod a+r /etc/apt/keyrings/docker.gpg
cass@UNIC:~$ echo   "deb [arch="$(dpkg --print-architecture)" signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu "$(. /etc/os-release && echo "$VERSION_CODENAME")" stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

cass@UNIC:~$ sudo apt-get update -q
...
cass@UNIC:~$ sudo apt-get install -q -y docker-ce docker-ce-cli containerd.io  docker-buildx-plugin docker-compose-plugin
...
*Command output*:
```
cass@UNIC:~$ sudo systemctl start docker
cass@UNIC:~$ sudo systemctl enable docker
...
```
cass@UNIC:~$ sudo gpasswd -a $USER docker
...
*Command output*:
```
cass@UNIC:~$ docker pull efabless/openlane:latest
...
*Command output*:
```
cass@UNIC:~$ python3 -m pip install --upgrade --no-cache-dir volare
...
*Command output*:
```
cass@UNIC:~$ echo 'export PATH=$HOME/.local/bin:$PATH' >> .bashrc
cass@UNIC:~$ source .bashrc
cass@UNIC:~$ volare --version
...
```
cass@UNIC:~$ export PDK_ROOT=$HOME/unic-cass/pdks
cass@UNIC:~$ export PDK=sky130A
cass@UNIC:~$ volare ls-remote --pdk sky130
...
*Command output*:
```
cass@UNIC:~$ volare enable --pdk sky130 6d4d11780c40b20ee63cc98e645307a9bf2b2ab8
...
```
cass@UNIC:~$ mkdir gcd_example
cass@UNIC:~$ cd gcd_example
cass@UNIC:~/gcd_example$ docker run -it -v $(pwd):/gcd_example/ -v $PDK_ROOT:$PDK_ROOT \
-e PDK_ROOT=$PDK_ROOT -u $(id -u $USER):$(id -g $USER) \
efabless/openlane:latest
...
```
cass@UNIC:~/gcd_example$ docker run -it -v $(pwd):/gcd_example -v $PDK_ROOT:$PDK_ROOT -e PDK=$PDK -e PDK_ROOT=$PDK_ROOT -u $(id -u $USER):$(id -g $USER) efabless/openlane:latest
